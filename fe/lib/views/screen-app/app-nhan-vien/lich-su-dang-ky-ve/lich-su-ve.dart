...
import '../../app-quan-tri/quan-ly-ve/xem-thong-tin-ve.dart';

class LichSuVeScreen extends StatefulWidget {
  const LichSuVeScreen({Key? key}) : super(key: key);

  @override
  _State createState() => _State();
}

class _State extends State<LichSuVeScreen> {
  //Load
    ...
    try {
      ...
      if (search != "") {
        response = await httpGet(
            "/api/ve-xe/get/page?filter=idNv:$id and $search&sort=status,desc&sort=createdDate&page=${curentPage - 1}&size=$selectedValueRpp",
            context);
      } else {
        response = await httpGet(
            "/api/ve-xe/get/page?filter=idNv:$id&sort=status,desc&sort=createdDate&page=${curentPage - 1}&size=$selectedValueRpp",
            context);
      }
                            ...
                            Row(
                              children: [
                                Container(
                                    ...
                                    decoration: BoxDecoration(
                                        color: Color.fromARGB(255, 9, 209, 26),
