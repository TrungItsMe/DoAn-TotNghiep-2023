...
class ThemMoiCTScreen extends StatefulWidget {
  final Function callBack;
  const ThemMoiCTScreen({Key? key, required this.callBack}) : super(key: key);

  @override
  _State createState() => _State();
}

class _State extends State<ThemMoiCTScreen> {
  //Load
                                ...
                                ElevatedButton(
                                  onPressed: () async {
                                    ...
                                    if (chonNgayStrat == null ||
                                      ...
                                      var checkCaTruc = await httpGet(
                                          "/api/ca-truc/get/page?filter=idBx:${selectedBx.id} and ca:${selectStatus.id} and startTime>:'${DateFormat('dd-MM-yyy').format(chonNgayStrat!)} 00:00:00' and startTime<'${DateFormat('dd-MM-yyy').format(chonNgayStrat!.add(Duration(days: 1)))} 00:00:00'",
                                          context);
                                      // print(
                                      //     "/api/ca-truc/get/page?filter=idBx:${selectedBx.id} and ca:${selectStatus.id} and startTime>:'${DateFormat('dd-MM-yyy').format(chonNgayStrat!)} 00:00:00' and startTime<'${DateFormat('dd-MM-yyy').format(chonNgayStrat!.add(Duration(days: 1)))} 00:00:00'");
                                      var bodyCT =
                                          jsonDecode(checkCaTruc["body"]);
                                      // print(bodyCT);
                                      if (bodyCT['result']['totalElements'] >
                                          0) {
                                        showToast(
                                          context: context,
                                          msg: "Nhân viên đã được phân công",
                                          color: Colors.orange,
                                          icon: const Icon(Icons.warning),
                                        );
                                        Navigator.pop(context);
                                        ...
                                        data.status = 0;
                                        // print(data.toJson());
                                        await httpPost("/api/ca-truc/post",
                                            data.toJson(), context);
                                        // print(abc);
